...
class TranslateOp extends TransformOp, WidgetToolOp {
  ...
  on_mousemove(event) {
    ...
    this.inputs.translation.set_data(vec);
    this.exec(td.ctx);
  }
  
  static gen_toolop(id, widget, ctx) {
    var op = new TranslateOp();
    var axis = new Vector3();
    
    axis[id] = 1.0;
    op.inputs.axis.set_data(axis);
    
    op.inputs.translation.add_listener(widget, function(widget, prop) {
      op.widget_on_tick(widget);
    });
  ...
  widget_on_tick(widget) {
    ...
    var co = this.transdata.center;
    var off = this.inputs.translation.data;
    
    mat.makeIdentity();
    mat.translate(co[0]+off[0], co[1]+off[1], co[2]+off[2]);
    
    if (this.inputs.datamode.data & EditModes.GEOMETRY)
      mat.multiply(this.transdata.object.matrix);
  }
}
TranslateOp.widget_axes = [1, 1, 1];
TranslateOp.widget_center = true;
TranslateOp.widget_align_normal = false;
