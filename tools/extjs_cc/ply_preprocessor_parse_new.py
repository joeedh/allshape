...
from js_sourcemap import SourceMap

tokens = (
  'CPP_COMMENT', 'CPP_ID',     'CPP_INTEGER', 
  'CPP_FLOAT',   'CPP_STRING', 'CPP_CHAR', 
  'CPP_WS',      'CPP_POUND',  'CPP_DPOUND'
)
...
class Preprocessor(object):
    def __init__(self,lexer=None):
        ...
        #maps old lines to new ones.  at end
        #of parse, sourcemap is inverted to 
        #map new lines to old ones.
        self.sourcemap = None #SourceMap
    ...
    def group_lines(self,input):
        ...
        while i < len(lines):
          ...
          while i < len(lines)-1 and lines[i].endswith("\\"):
            line2 = lines[i+1]
            if line2.endswith("\\"):
              line2 = line2[:-1]
        ...
        for toks in tlines:
          ...
          if not in_comment:
            ...
            for i, t in enumerate(toks):
              if t.value == "*" and lastt != None and lastt.value == "/":
                in_comment = True
                yield toks[:i-1]
              lastt = t 
          else:
    ...
    def parsegen(self, input, source=None):
      #kill \r's
      #done in calling code now
      #input = input.replace("\r", "")
        
      for tok in self.parsegen_intern(input, source):
        if tok.type in self.t_WS and "\n" in tok.value:
          self.newlineno += 1
          #tok.value = str(self.newlineno) + tok.value
        
        #print(tok.lexpos, len(self.sourcemap.map))
        self.sourcemap.insert(tok.lexpos, tok.lineno, source, self.newlineno, self.filename)
        yield tok
        
    def parsegen_intern(self, input, source=None):
        ...
        while xi < len(lines)-1:
            ...
            if tok.value == '#':
                ...
                tok1.type = self.t_WS
                tok1.value = "\n"
                tok1.lexpos = tok.lexpos
                tok1.lineno = tok.lineno 
    ...
    def parse(self, input,source=None,ignore={}):
        self.sourcemap = SourceMap(input, source)
    ...
    def token(self):
        try:
            while True:
                ...
                #long-form comment
                #if tok.value == "*" and self.last != None and self.last.value == "/":
                #  tok = next(self.parser)
                #  while tok != None and 
